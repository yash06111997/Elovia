...
router.post(
  ...
  async (req, res) => {
    ...
    if (!imageBase64 || typeof imageBase64 !== "string") {
      res
        .status(400)
        .json({ error: "imageBase64 is required", code: "bad_request" });
      return;
    }
    ...
    if (approxBytes > MAX_IMAGE_BYTES) {
      res.status(413).json({
        error:
          "That image is too large. Please retake it at a lower resolution.",
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!profile || typeof profile !== "object") {
      res
        .status(400)
        .json({ error: "profile is required", code: "bad_request" });
      return;
    }

    const resolvedPlanType: "daily" | "scheduled" =
      planType === "daily" ? "daily" : "scheduled";
    const { system, prompt } = buildWorkoutPrompt(
      profile,
      resolvedPlanType,
      preferences,
    );
...
router.post(
  "/generate-meal-plan",
  gateAiRoute("generate-meal-plan"),
  async (req, res) => {
    ...
    const { system, prompt, dietType } = buildMealPlanPrompt(
      profile,
      dietPrefs,
    );
...
router.post("/coach-chat", gateAiRoute("coach-chat"), async (req, res) => {
  ...
  if (!Array.isArray(messages) || messages.length === 0) {
    res
      .status(400)
      .json({ error: "messages is required", code: "bad_request" });
    return;
  }
