...
const MINIMUM_POSTGRES_VERSION_NUMBER = MINIMUM_POSTGRES_MAJOR * 10_000;
...
async function verifyBaselineAdoption(client, sql) {
  ...
  try {
    ...
    try {
      await client.query(
        `SET LOCAL search_path TO ${quotedTarget}, pg_catalog`,
      );
...
export async function runMigrations(
  ...
  try {
    ...
    const versionNumber = Number(serverVersion.rows[0]?.version_number);
    if (
      !Number.isSafeInteger(versionNumber) ||
      versionNumber < MINIMUM_POSTGRES_VERSION_NUMBER
    ) {
      throw new Error(
        `PostgreSQL ${MINIMUM_POSTGRES_MAJOR} or newer is required.`,
      );
    }
